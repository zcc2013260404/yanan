<template>
	<div class="box">
		<div class="top">
            <div>
                <span class="backMain" @click="cancelOpt">租户管理</span>
                <span class="el-icon-arrow-right">
                    <span></span>分配权限
                </span>
            </div>
            <div>{{tenantObj.name}}</div>
        </div>
	...
	export default {
		...
		methods: {
			...
			getAuthData() {
				this.$axios.leansite({
					method: 'get',
					url: this.API.leansite.getTenantPower,
					params: {
						'tenantId': this.tenantObj.id * 1
					}
				}).then((res) => {
			...
			save() {
				...
				this.$axios.leansite({
					method: 'POST',
					url: this.API.leansite.saveTenantPower,
					params: {
						'tenantId': this.tenantObj.id * 1,
				...
				this.$emit("TenantPowerCallBack", false);
			},
			/**
			 * 获取所有被选中的权限的id
			 */
